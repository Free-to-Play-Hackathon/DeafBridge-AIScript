...
public class GroqConversationAgent : IConversationAgent
{
    private const string Endpoint = "https://api.groq.com/openai/v1/chat/completions";
    private readonly IHttpClientFactory _httpClientFactory;
    private readonly IConfiguration _configuration;
    private readonly ILogger<GroqConversationAgent> _logger;
    private readonly FakeConversationAgent _fallbackAgent = new();

    public GroqConversationAgent(
        IHttpClientFactory httpClientFactory,
        IConfiguration configuration,
        ILogger<GroqConversationAgent> logger)
    {
    ...
    public async Task<AgentResult> AnalyzeAsync(AgentContext context, CancellationToken cancellationToken)
    {
        ...
        using var client = _httpClientFactory.CreateClient(nameof(GroqConversationAgent));
        using var response = await client.SendAsync(httpRequest, cancellationToken);
        var body = await response.Content.ReadAsStringAsync(cancellationToken);

        if (!response.IsSuccessStatusCode)
        {
            _logger.LogWarning("Groq agent failed with status {StatusCode}; using fake fallback", (int)response.StatusCode);
            return await _fallbackAgent.AnalyzeAsync(context, cancellationToken);
        }
        ...
        {
            _logger.LogWarning("Groq agent returned empty content; using fake fallback");
            return await _fallbackAgent.AnalyzeAsync(context, cancellationToken);
        }
        ...
        {
            _logger.LogWarning(ex, "Groq agent returned invalid structured JSON; using fake fallback");
            return await _fallbackAgent.AnalyzeAsync(context, cancellationToken);
        }
    }
        ...
        return new
        {
            ...
            messages = new object[]
            {
                new
                {
                    ...
                    - Only propose tools supported by explicit transcript evidence or clear memory context.
                    ...
                    - Avoid duplicate or conflicting tools based on memory.
    ...
    {
        var dto = JsonSerializer.Deserialize<GroqAgentResultDto>(content, JsonOptions)
    ...
    private static AgentAction MapAction(GroqAgentActionDto action)
    {
    ...
    private sealed class GroqAgentResultDto
    {
        ...
        public List<GroqAgentActionDto>? Actions { get; set; }
    }

    private sealed class GroqAgentActionDto
    {
