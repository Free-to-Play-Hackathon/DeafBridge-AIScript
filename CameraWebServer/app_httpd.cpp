...
void setupLedFlash(int pin) {
  ...
  bool attached = ledcAttach(pin, 5000, 8);

  if (!attached) {
    log_e("Failed to attach LED flash PWM to GPIO %d", pin);
  } else {
    log_i("LED flash PWM attached to GPIO %d", pin);
  }
#else
  log_i("LED flash disabled: LED_GPIO_NUM is not defined");
#endif
}
