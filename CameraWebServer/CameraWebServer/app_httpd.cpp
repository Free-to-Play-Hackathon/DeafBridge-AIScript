...
static esp_err_t play_handler(httpd_req_t *req) {
  ...
  while (remaining > 0) {
    ...
    remaining -= ret;
    audioI2S.write((uint8_t *)buf, ret);
  }
...
static esp_err_t oled_handler(httpd_req_t *req) {
  if (!oledReady) {
    httpd_resp_send_err(req, HTTPD_500_INTERNAL_SERVER_ERROR, "OLED not ready");
    return ESP_FAIL;
  }
  ...
  // Split text into up to 4 lines of ~21 chars each for 128x64 OLED
  String text(buf);
  free(buf);
  ...
  for (int i = 0; i <= (int)text.length() && lineIdx < 4; i++) {
    if (i == (int)text.length() || (i - start >= 21 && text.charAt(i) == ' ') || text.charAt(i) == '\n') {
  ...
  if (lineIdx < 4 && start < (int)text.length()) {
    lines[lineIdx] = text.substring(start);
    lines[lineIdx].trim();
  }

  showOLED(lines[0], lines[1], lines[2], lines[3]);
  log_i("OLED display: %s", text.c_str());
